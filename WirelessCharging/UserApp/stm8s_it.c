...
*/
  ...
  */
/**
  static unsigned char noFinish =0, Last_State=0,time1 =0,time2 =0;
  ...
  if(noFinish){
    if(Last_State){             //上次未处理完的电平是高电平
        ...
        if(TIM4->SR1 & 0x01){   //溢出跳出的等待
          ...
          if((time1+time2)>100 ){               //如果T1加T2大于400us
                //保存一个1
              TIM4_Close();
          }
        }
...
BitStatus GPIO_ReadInPin(GPIO_TypeDef* GPIOx, GPIO_Pin_TypeDef GPIO_Pin)
{
